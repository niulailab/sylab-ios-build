...
function ChatDetailScreenInner() {
  ...
  // [v123] MVCP must turn on only after the whole list has finished settling (images/cards can
  // resolve well after the first text layout). A fixed 120ms timer (v122) could fire while async
  // heights were still being injected, so MVCP wrongly translated those late height changes as
  // "content inserted above the anchor" -> huge top gap, then a corrective jump at the bottom.
  // We (re)arm a debounce on every content-size change and enable MVCP only after the content
  // size has stayed unchanged for SETTLE_MS, independent of which content caused the reflow.
  const MVCP_SETTLE_MS = 400;
  const mvcpSettleTimerRef = useRef<any>(null);
  const armMvcpSettle = React.useCallback(() => {
    if (mvcpSettleTimerRef.current) { clearTimeout(mvcpSettleTimerRef.current); mvcpSettleTimerRef.current = null; }
    mvcpSettleTimerRef.current = setTimeout(() => {
      mvcpSettleTimerRef.current = null;
      setEnableMvcp(true);
    }, MVCP_SETTLE_MS);
  }, []);
  const tryInitialAnchor = () => {
    ...
    requestAnimationFrame(() => {
      ...
      requestAnimationFrame(() => {
        ...
        // Start watching for layout quiescence; MVCP enabled only once content size is stable.
        armMvcpSettle();
      });
    });
  };
