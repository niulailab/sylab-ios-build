...
def get_sbx_dir(conversation_id: str) -> Path:
    """沙箱工具层写入的目录：sbx_conv-<id>（只读引用，不主动创建）"""
    safe_id = "".join(c for c in conversation_id if c.isalnum() or c in "-_")
    if not safe_id:
        return None
    return BASE_DIR / ("sbx_conv-" + safe_id)

def get_conv_dirs(conversation_id: str, create_plain: bool = True):
    """
    [FIX v4.1-Bug8] AI 沙箱产物写入 sbx_conv-<id>/，客户端项目页按 <id>/ 查询，
    命名空间错位导致项目文件列表为空。这里统一返回两个目录的合并视图：
    列表/下载/删除同时可见两边；上传仍只写用户目录 <id>/。
    顺序：[用户目录, 沙箱目录]；沙箱目录不存在则只返回用户目录。
    """
    plain = get_conv_dir(conversation_id) if create_plain else (lambda: (
        BASE_DIR / "".join(c for c in conversation_id if c.isalnum() or c in "-_")
    ))()
    if plain is None:
        return []
    dirs = [plain]
    sbx = get_sbx_dir(conversation_id)
    if sbx is not None and sbx.exists() and sbx.is_dir() and sbx.resolve() != plain.resolve():
        dirs.append(sbx)
    return dirs
...
class FileHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
            ...
            if BASE_DIR.exists():
                ...
                for conv_dir in BASE_DIR.iterdir():
                    if not conv_dir.is_dir():
                        continue
                    name = conv_dir.name
                    # [FIX v4.1-Bug8] 沙箱目录计入真实会话，不单独出现 sbx_conv-* 键
                    if name.startswith("sbx_conv-"):
                        continue
                    file_count, total_size = _dir_stat(conv_dir)
                    sbx = BASE_DIR / ("sbx_conv-" + name)
                    if sbx.is_dir():
                        sc, ss = _dir_stat(sbx)
                        file_count += sc
                        total_size += ss
        ...
        if parsed.path == "/api/files":
            ...
            conv_dirs = get_conv_dirs(conv_id)
            if not conv_dirs:
    ...
    def do_DELETE(self):
        ...
        if len(parts) == 4 and parts[0] == "api" and parts[1] == "files":
            ...
            conv_dirs = get_conv_dirs(conv_id, create_plain=False)
            removed = 0
            for d in conv_dirs:
                try:
                    cand = d / filename
