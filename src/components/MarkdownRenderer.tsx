  ...
  const parseInline = (text: string, key: string): React.ReactNode => {
    ...
    while (remaining.length > 0) {
      ...
      if (first.type === 'bold') {
        ...
        parts.push(<Text key={`${key}-l${k}`} style={{ color: '#2563eb', textDecorationLine: 'underline' }} onPress={() => { const u=first.m[2]; if (pickFileInfo(u)) { downloadAndShare(u); } else { openExternally(u); } }}>{first.m[1]}</Text>);
        remaining = remaining.slice(first.idx + first.m[0].length);
      }
      k++;
    }
